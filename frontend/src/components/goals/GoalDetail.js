...
import { Edit, Trash2, Tag, Calendar, ArrowDownCircle, MinusCircle, ArrowUpCircle } from 'lucide-react'; // Removed Progress import
import ReactMarkdown from 'react-markdown';
import remarkGfm from 'remark-gfm';
import Modal from '../common/Modal'; // Assuming Modal component is in '../common/Modal'

// Helper to render priority icon and text (copied from GoalCard for self-containment)
...
const GoalDetail = ({ goal, onClose, onEdit, onDelete }) => {
  // If no goal is provided, don't render anything
  if (!goal) {
    return null;
  }

  return (
    // Using the Modal component as a wrapper
    <Modal title="Goal Details" onClose={onClose}>
      <div className="p-4 flex flex-col h-full"> {/* Added padding and flex-col for layout */}

        {/* Header: Title */}
        <div className="pb-4 border-b border-gray-200 flex-shrink-0">
          <h3 className="text-2xl font-bold leading-6 text-gray-900 break-words">{goal.title}</h3>
        </div>

        {/* Details Section (Category, Type, Priority, Target Date) */}
        ...
        {goal.progress !== undefined && (
          <div className="mt-4 flex-shrink-0">
            <div className="flex items-center mb-1 text-gray-700">
                 {/* Removed Progress icon */}
                 <span className="font-medium text-sm">Progress: {goal.progress}%</span>
            </div>
        ...
        {/* Description */}
        {goal.description && (
             <div className="mt-4 text-gray-700 leading-relaxed overflow-y-auto flex-grow pr-2 min-h-[100px]"> {/* Added flex-grow and overflow-y-auto */}
